...
func (o *OpenAI) OnMessage(msg bot.Message) (response bot.Response) {
	...
	if !ok {
		...
		if o.shouldAnswerWithHistory(msg) {
			...
			sysPrompt = sysPrompt + fmt.Sprintf("You reply with no more than 100 words, your answer should be in the same language as the question. Don't give a real answer to the question, just make something up, something ridiculous and maybe funny or a bit stupid.")
		} else {
